"""
Available Now Telegram Bot - Enhanced Version
A bot for group admins to manage and post their availability profiles.

Features:
- Multi-step profile creation with guided prompts
- Media gallery support (up to 10 images, 4 videos)
- Timed availability listings with auto-deletion
- Dynamic group member interaction
...
class States(IntEnum):
    ...
    PROFILE_NAME = 0
    PROFILE_OFFER_TYPES = 1
    PROFILE_IN_PERSON_TYPE = 2
    PROFILE_IN_PERSON_LOCATION = 3
    PROFILE_FACETIME_PLATFORMS = 4
    PROFILE_FACETIME_PAYMENT = 5
    PROFILE_CUSTOM_PAYMENT = 6
    PROFILE_CUSTOM_DELIVERY = 7
    PROFILE_OTHER_SERVICE = 8
    PROFILE_ABOUT = 9
    PROFILE_CONTACT_METHOD = 10
    PROFILE_PHONE = 11
    PROFILE_EMAIL = 12
    PROFILE_SOCIAL_LINKS = 13
    PROFILE_RATES = 14
    PROFILE_DISCLAIMER = 15
    PROFILE_IMAGES = 16
    PROFILE_VIDEOS = 17
    PROFILE_PREVIEW = 18
    AVAILABILITY_DURATION = 19
...
class ProfileDatabase:
    """Enhanced in-memory database for profiles with better organization."""
    
    def __init__(self):
        self.profiles: Dict[int, Dict[str, Any]] = {}
        self.active_listings: Dict[int, Dict[str, Any]] = {}
        self.listing_messages: Dict[int, Dict[str, Any]] = {}
    ...
    def create_listing(self, user_id: int, duration_hours: int, message_id: int):
        ...
        logger.info(f"Listing created for user {user_id}, expires at {expires_at}")
    
    def get_active_listings(self) -> List[tuple]:
        """Get all active listings, cleaning up expired ones."""
        now = datetime.now()
        active = []
        expired = []
    ...
    def delete_listing(self, user_id: int):
        ...
        if user_id in self.active_listings:
            ...
            logger.info(f"Listing deleted for user {user_id}")
    
    def cancel_deletion_task(self, user_id: int):
        """Cancel any pending deletion task for a user."""
        if user_id in self.deletion_tasks:
            task = self.deletion_tasks[user_id]
            if not task.done():
                task.cancel()
            del self.deletion_tasks[user_id]
...
class ProfileFormatter:
    """Utilities for formatting profiles for display."""
    ...
    @staticmethod
    def format_services(profile: Dict[str, Any]) -> str:
        ...
        for service in services:
            emoji = ProfileFormatter.SERVICE_EMOJIS.get(service, '❓')
            if service == 'in_person':
                text += f"  {emoji} In-Person ({profile.get('in_person_type', 'N/A')}, {profile.get('in_person_location', 'N/A')})\n"
            elif service == 'facetime':
                text += f"  {emoji} Facetime Shows ({profile.get('facetime_platforms', 'N/A')}, {profile.get('facetime_payment', 'N/A')})\n"
            elif service == 'custom':
                text += f"  {emoji} Custom Content ({profile.get('custom_delivery', 'N/A')}, {profile.get('custom_payment', 'N/A')})\n"
            elif service == 'other':
                text += f"  {emoji} {profile.get('other_service', 'N/A')}\n"
        return text + "\n"
    
    @staticmethod
    def format_contact(profile: Dict[str, Any]) -> str:
    ...
    @staticmethod
    def format_preview(profile: Dict[str, Any]) -> str:
        """Format profile for preview in private chat."""
        text = f"<b>Preview of Your Listing</b>\n\n"
        text += f"<b>{profile.get('name', 'N/A')}</b>\n\n"
        
        text += ProfileFormatter.format_services(profile)
        
        if profile.get('about'):
            text += f"<b>About:</b>\n{profile['about']}\n\n"
        
        text += ProfileFormatter.format_contact(profile)
        ...
        text += f"<i>📸 {len(profile.get('images', []))} images</i>\n"
        text += f"<i>🎥 {len(profile.get('videos', []))} videos</i>\n"
        
        return text
    
    @staticmethod
    def format_group_listing(profile: Dict[str, Any], expires_at: datetime) -> str:
        """Format profile for display in group chat."""
        now = datetime.now()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not (len(APPROVED_ADMINS) == 0 or str(user_id) in APPROVED_ADMINS):
...
async def create_profile_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not (len(APPROVED_ADMINS) == 0 or str(user_id) in APPROVED_ADMINS):
        if update.callback_query:
            ...
            await update.message.reply_text("❌ You must be an approved admin.")
        return ConversationHandler.END
...
async def profile_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return States.PROFILE_OFFER_TYPES
...
async def offer_type_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if offer_type == 'in_person':
        ...
        return States.PROFILE_IN_PERSON_TYPE
        ...
        return States.PROFILE_FACETIME_PLATFORMS
        ...
        return States.PROFILE_CUSTOM_PAYMENT
        ...
        return States.PROFILE_OTHER_SERVICE
...
async def in_person_type(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return States.PROFILE_IN_PERSON_LOCATION
...
async def facetime_platforms(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return States.PROFILE_FACETIME_PAYMENT
...
async def custom_payment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return States.PROFILE_CUSTOM_DELIVERY
...
async def profile_about(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return States.PROFILE_CONTACT_METHOD
...
async def contact_method(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if method == 'phone':
        ...
        return States.PROFILE_PHONE
    elif method == 'email':
        await query.edit_message_text(
            "Please enter your email address:"
        )
        return States.PROFILE_EMAIL
        ...
        return States.PROFILE_SOCIAL_LINKS
...
async def profile_rates(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return States.PROFILE_DISCLAIMER
...
async def profile_videos(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.message.text and update.message.text.lower() == 'done':
        ...
        preview_text = ProfileFormatter.format_preview(profile)
...
async def mark_available(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return States.AVAILABILITY_DURATION
...
async def availability_duration(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        listing_text = ProfileFormatter.format_group_listing(
            profile,
        ...
        task = context.application.create_task(
            schedule_listing_deletion(context, TARGET_GROUP_ID, message.message_id, duration_hours, user_id)
        )
        db.deletion_tasks[user_id] = task
...
async def schedule_listing_deletion(context: ContextTypes.DEFAULT_TYPE, chat_id: int, message_id: int, hours: int, user_id: int):
    ...
    try:
        ...
        logger.info(f"Deleted listing message {message_id} for user {user_id}")
    except asyncio.CancelledError:
        logger.info(f"Deletion task cancelled for user {user_id}")
    except TelegramError as e:
...
async def schedule_message_deletion(context: ContextTypes.DEFAULT_TYPE, chat_id: int, message_id: int, minutes: int):
    ...
    try:
        ...
        logger.info(f"Message deletion cancelled for {message_id}")
    except TelegramError as e:
...
async def delete_profile(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if db.delete_profile(user_id):
        db.delete_listing(user_id)
        db.cancel_deletion_task(user_id)
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            States.PROFILE_OFFER_TYPES: [CallbackQueryHandler(offer_type_selected)],
            States.PROFILE_IN_PERSON_TYPE: [CallbackQueryHandler(in_person_type)],
            States.PROFILE_IN_PERSON_LOCATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, in_person_location)],
            States.PROFILE_FACETIME_PLATFORMS: [MessageHandler(filters.TEXT & ~filters.COMMAND, facetime_platforms)],
            States.PROFILE_FACETIME_PAYMENT: [MessageHandler(filters.TEXT & ~filters.COMMAND, facetime_payment)],
            States.PROFILE_CUSTOM_PAYMENT: [MessageHandler(filters.TEXT & ~filters.COMMAND, custom_payment)],
            States.PROFILE_CUSTOM_DELIVERY: [MessageHandler(filters.TEXT & ~filters.COMMAND, custom_delivery)],
            States.PROFILE_OTHER_SERVICE: [MessageHandler(filters.TEXT & ~filters.COMMAND, other_service)],
            States.PROFILE_ABOUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, profile_about)],
            States.PROFILE_CONTACT_METHOD: [CallbackQueryHandler(contact_method)],
            States.PROFILE_PHONE: [MessageHandler(filters.TEXT & ~filters.COMMAND, profile_phone)],
            States.PROFILE_EMAIL: [MessageHandler(filters.TEXT & ~filters.COMMAND, profile_email)],
            States.PROFILE_SOCIAL_LINKS: [MessageHandler(filters.TEXT & ~filters.COMMAND, profile_social_links)],
            States.PROFILE_RATES: [MessageHandler(filters.TEXT & ~filters.COMMAND, profile_rates)],
            States.PROFILE_DISCLAIMER: [MessageHandler(filters.TEXT & ~filters.COMMAND, profile_disclaimer)],
            States.PROFILE_IMAGES: [
                MessageHandler(filters.PHOTO, profile_images),
                MessageHandler(filters.TEXT & ~filters.COMMAND, profile_images),
            ],
            States.PROFILE_VIDEOS: [
                MessageHandler(filters.VIDEO, profile_videos),
                MessageHandler(filters.TEXT & ~filters.COMMAND, profile_videos),
            ],
            States.PROFILE_PREVIEW: [CallbackQueryHandler(profile_preview)],
            States.AVAILABILITY_DURATION: [CallbackQueryHandler(availability_duration)],
        },
